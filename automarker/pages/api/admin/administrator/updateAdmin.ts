...
import { modifyAdmin } from "../../../../lib/models/user";
    ...
    try {
        ...
        if (!session) {
            ...
            if (session.role === "ADMIN") {
                try {
                    const up = await modifyAdmin(req.body.adminId, req.body.name, req.body.lastName, req.body.username, req.body.password);
                    if (!up) {
                        res.status(404).json();
                    }
                    else {
